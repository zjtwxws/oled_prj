/**
 * @file    boot_proto.c
 * @brief   Bootloader 帧协议引擎实现
 *
 * 帧格式: SOF(0xA5) LEN CMD DATA[0..LEN-1] CRC8 EOF(0x5A)
 * LEN 仅计 DATA 段字节数 (不含 CMD)。
 * CRC-8-ATM, 多项式 0x07, 查表法。
 */

#include "boot_proto.h"
#include <string.h>

/* CRC-8-ATM 查表 */
static const uint8_t crc8_table[256] = {
    
};

typedef enum
{
    S_WAIT_SOF = 0,
    S_WAIT_LEN,
    S_WAIT_CMD,
    S_WAIT_DATA,
    S_WAIT_CRC,
    S_WAIT_EOF
} rx_state_t;

static rx_state_t    s_state = S_WAIT_SOF;
static boot_frame_t  s_frame;
static uint8_t       s_data_idx;
static uint8_t       s_expect_len;
static uint8_t       s_crc_acc;

static uint8_t tx_buf[PROTO_FRAME_MAX];

/**
 * @brief  逐字节喂入协议解析器
 * @param  byte  接收到的字节
 * @return 1=完整帧已解码
 */
int boot_proto_feed(uint8_t byte)
{
    switch (s_state)
    {
    case S_WAIT_SOF:
        if (byte == PROTO_SOF)
        {
            s_crc_acc = crc8_table[0x00 ^ byte];
            s_state = S_WAIT_LEN;
        }
        break;

    case S_WAIT_LEN:
        if (byte > PROTO_MAX_DATA)
        {
            s_state = S_WAIT_SOF;
        }
        else
        {
            s_expect_len = byte;
            s_frame.len = byte;
            s_crc_acc = crc8_table[s_crc_acc ^ byte];
            s_state = S_WAIT_CMD;
        }
        break;

    case S_WAIT_CMD:
        s_frame.cmd = byte;
        s_crc_acc = crc8_table[s_crc_acc ^ byte];
        if (s_expect_len == 0)
        {
            s_state = S_WAIT_CRC;
        }
        else
        {
            s_data_idx = 0;
            s_state = S_WAIT_DATA;
        }
        break;

    case S_WAIT_DATA:
        s_frame.data[s_data_idx++] = byte;
        s_crc_acc = crc8_table[s_crc_acc ^ byte];
        if (s_data_idx >= s_expect_len)
        {
            s_state = S_WAIT_CRC;
        }
        break;

    case S_WAIT_CRC:
        if (byte == s_crc_acc)
        {
            s_state = S_WAIT_EOF;
        }
        else
        {
            s_state = S_WAIT_SOF;
        }
        break;

    case S_WAIT_EOF:
        if (byte == PROTO_EOF)
        {
            s_state = S_WAIT_SOF;
            return 1;
        }
        s_state = S_WAIT_SOF;
        break;
    }
    return 0;
}

/**
 * @brief  获取最近解码成功的帧
 */
const boot_frame_t* boot_proto_get_frame(void)
{
    return &s_frame;
}

/**
 * @brief  复位接收状态机
 */
void boot_proto_reset(void)
{
    s_state = S_WAIT_SOF;
}

/**
 * @brief  构建发送帧
 * @param  cmd   命令码
 * @param  data  负载数据 (可为 NULL 当 len=0)
 * @param  len   数据长度
 * @return 帧总字节数
 */
uint16_t boot_proto_build(uint8_t cmd, const uint8_t *data, uint8_t len)
{
    uint8_t idx = 0;

    tx_buf[idx++] = PROTO_SOF;
    tx_buf[idx++] = len;
    tx_buf[idx++] = cmd;

    if (len > 0 && data != NULL)
    {
        memcpy(&tx_buf[idx], data, len);
        idx += len;
    }

    /* CRC-8: 覆盖 SOF 到 DATA 末 */
    uint8_t crc = 0x00;
    for (uint8_t i = 0; i < idx; i++)
    {
        crc = crc8_table[crc ^ tx_buf[i]];
    }
    tx_buf[idx++] = crc;
    tx_buf[idx++] = PROTO_EOF;

    return idx;
}

/**
 * @brief  获取发送缓冲区指针
 */
const uint8_t* boot_proto_tx_buf(void)
{
    return tx_buf;
}
