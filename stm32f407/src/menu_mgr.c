...
{
    if (page >= SSD1306_PAGES - 1 || x >= SSD1306_WIDTH)
    {
        return x;
    }
...
static uint8_t draw_text(uint8_t page, uint8_t x, const char *text, bool inverted)
{
    ...
    while (*p && x < SSD1306_WIDTH)
    {
        ...
        {
            /* 3 字节 UTF-8 中文 */
            if (x + FONT_CHINESE_W > SSD1306_WIDTH)
            {
                break;
            }
            if (inverted)
        ...
        {
            /* ASCII */
            if (x + FONT_ASCII_W > SSD1306_WIDTH)
            {
                break;
            }
            if (inverted)
...
/**
 * @brief  绘制菜单项后缀（TOGGLE [x]/[ ] 或 VALUE [NNN]）
 * @param  page 参数说明
 * @param  item 参数说明
 * @param  inverted 参数说明
 * @date   2026-08-07
...
static void draw_suffix(uint8_t page, const menu_item_t *item, bool inverted)
{
    ...
    } else if (item->type == MENU_TYPE_VALUE)
    {
        ...
        if (val >= 100)
        {
            suffix[idx++] = '0' + (val / 100);
        }
        if (val >= 10)
        {
            suffix[idx++] = '0' + ((val / 10) % 10);
        }
        suffix[idx++] = '0' + (val % 10);
        suffix[idx++] = ']';
...
static void render_menu(void)
{
    ...
    for (uint8_t row = 0; row < MENU_ROWS_VISIBLE; row++)
    {
        ...
        if (item_idx >= g_menu.current_menu_count)
        {
            break;
        }
...
{
    if (g_menu.current_menu_count == 0)
    {
        return;
    }
    if (g_menu.cursor > 0)
