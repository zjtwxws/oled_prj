...
static USART_HandleTypeDef *p_debug_uart = NULL;

/* ---- 初始化 ---- */

void debug_console_init(void *huart)
{
    p_debug_uart = (USART_HandleTypeDef *)huart;
...
void debug_console_write(const char *str)
{
    if (p_debug_uart && str) {
        ...
        if (len > 0) {
            HAL_USART_Transmit(p_debug_uart, (uint8_t *)str, len, HAL_MAX_DELAY);
        }
    }
}
...
void debug_printf(const char *fmt, ...)
{
    ...
    if (len > 0 && (uint16_t)len < sizeof(buf)) {
        HAL_USART_Transmit(p_debug_uart, (uint8_t *)buf, (uint16_t)len, HAL_MAX_DELAY);
    }
    /* 追加换行 */
    HAL_USART_Transmit(p_debug_uart, (uint8_t *)"\r\n", 2, HAL_MAX_DELAY);
}
...
void debug_hexdump(const uint8_t *data, uint16_t len)
{
    ...
    for (uint16_t i = 0; i < len; i++) {
        ...
        HAL_USART_Transmit(p_debug_uart, (uint8_t *)hex, 3, HAL_MAX_DELAY);
