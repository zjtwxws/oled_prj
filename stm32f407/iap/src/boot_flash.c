...
int boot_erase_sectors(const uint32_t *sectors, uint8_t count)
{
    ...
    {
        if (boot_erase_sector(sectors[i]) != 0)
        {
            return -1;
        }
    }
...
{
    const uint32_t slot_a_sectors[] = SLOT_A_SECTORS;
    const uint32_t slot_b_sectors[] = SLOT_B_SECTORS;

    if (slot == 0)
    {
        return boot_erase_sectors(slot_a_sectors, SLOT_A_SECTOR_COUNT);
    }
    else
    {
        return boot_erase_sectors(slot_b_sectors, SLOT_B_SECTOR_COUNT);
    }
}
...
static const uint32_t crc32_table[256] = {
    ...
    0x7807C9A2,0x0F00F934,0x9609A88F,0xE10E9818,0x7F6A0DBB,0x086D3D2D,
...
{
    uint32_t base = (slot == 0) ? 0x08008000UL : 0x08060000UL;
    ...
    uint32_t crc = 0xFFFFFFFF;
    const uint8_t *addr = (const uint8_t *)base;

    for (uint32_t i = 0; i < size; i++)
    {
        crc = (crc >> 8) ^ crc32_table[(crc ^ addr[i]) & 0xFF];
    }
