...
| 按键 | 物理引脚 | 短按 | 长按 (≥2s) |
|------|---------|------|-----------|
| KEY1 | PE1 | **下** — 光标下移 / 值增加 (实现中KEY1=下,与原始设计相反) | 快速连续下翻 (每 150ms 重复) |
| KEY2 | PE2 | **上** — 光标上移 / 值减少 (实现中KEY2=上,与原始设计相反) | 快速连续上翻 (每 150ms 重复) |
...
> **实现差异**: KEY1/KEY2 的上/下方向在代码中与本文档原始设计相反 (KEY1=下, KEY2=上)，以实际代码为准。对比度设置从 2 级改为 3 级 (设置→对比度设置→对比度)。
...
| 设置→对比度设置→对比度 (3级菜单) | `ssd1306_set_contrast(value)` |
